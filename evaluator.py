"""
Given completions and a correct numerical answer, calculates rewards.
"""
...
class FinanceEvaluator(RewardEvaluator):
    ...
    def compute_rewards(
        ...
        soft_format_scores = self._soft_format_reward(completions)
        xml_count_scores = self._xml_count_reward(completions)
        ...
        rewards_per_func[:, 2] = torch.tensor(soft_format_scores, dtype=torch.float32, device=device)
        rewards_per_func[:, 3] = torch.tensor(xml_count_scores, dtype=torch.float32, device=device)
        ...
        metrics = {
            ...
            "rewards/soft_format_func": reward_per_func[2].item(),
            "rewards/xml_count_func": reward_per_func[3].item(),
    ...
    def get_reward_breakdown(self, reward_scores: torch.Tensor) -> Dict[str, float]:
        ...
        return {
            'answer_format': reward_scores[0].item(),
...
class GSM8kEvaluator(RewardEvaluator):
    ...
    def _extract_xml_answer(self, text: str) -> str:
        ...
        # Remove non-numerical characters to get a clean number
        answer = ''.join(char for char in answer if char.isdigit() or char == '.')
        
        # Convert to float if possible, otherwise return empty string
        try:
